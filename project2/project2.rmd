...
vars <- setdiff(colnames(df), "High.Income")

cat_var <- vars[
  sapply(df[, vars], class) %in% c("factor", "character", "logical")
]
num_var <- vars[
  sapply(df[, vars], class) %in% "numeric"
]

print(paste("There are", as.character(length(cat_var)), "categorical features,", as.character(length(num_var)), "numerical features, and 1 target.", sep = " "))
...
Firstly, we will compute a null model.
