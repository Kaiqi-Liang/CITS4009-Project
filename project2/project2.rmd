...
clust_df = df
  # df %>% filter(Estimate.Base.Salary < 150000)
head(clust_df)
```
...
print_clusters(clust_df, groups, cols_to_print)
clust_df$group <- as.factor(groups)
```

```{r}

ggplot(clust_df, aes(Estimate.Base.Salary, fill=group)) + 
  geom_histogram()
...
hclust.project2D <- cbind(project2D, cluster=as.factor(groups), state=df$State)
