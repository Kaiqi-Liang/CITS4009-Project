...
paste(
  "There are",
  nrow(df[df$High.Income, ]),
  "high income observations.",
  "There are",
  nrow(df[!df$High.Income, ]),
...
categorical_variables <- variables[
  sapply(df[, variables], class) %in% c("factor", "character", "logical")
]

numerical_variables <- variables[
  sapply(df[, variables], class) %in% "numeric"
]
...
```{r}
null_model <- sum(training_set$High.Income) / nrow(training_set)
(calc_auc(rep(null_model, nrow(test_set)), test_set$High.Income))
(log_likelihood(null_model, training_set$High.Income))
```
...
cross_validation_100_fold <- function(variable) {
  ...
  for (i in seq(aucs)) {
    ...
    pred_col <- single_variable_prediction(
      ...
      training_set[split, "High.Income"],
      training_set[!split, variable]
    )
    aucs[i] <- calc_auc(pred_col, training_set[!split, "High.Income"])
  }
  mean_auc <- mean(aucs)
  if (mean_auc > 0.65) {
...
for (variable in numerical_variables) {
  ...
  if (variable == "Friend.Recommendation") {
    training_set[categorical_variable] <- cut(
  ...
  cross_validation_100_fold(categorical_variable)
}
...
roc_plot <- function(pred_col, out_col, colour, overlaid = FALSE) {
  par(new = overlaid)
  plot(
...
roc_plot(title_pred, test_set$High.Income, "orange")
roc_plot(company_pred, test_set$High.Income, "blue", TRUE)
```
...
caret::train(
  ...
  as.factor(training_set$High.Income),
  "naive_bayes"
)
```
...
example <- test_set[c(1, 3), variables]
...
caret::train(
  x = training_set[variables],
  y = as.factor(training_set$High.Income),
  method = "glm",
