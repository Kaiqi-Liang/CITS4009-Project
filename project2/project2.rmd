...
(log_likelihood(null_model, train$High.Income))
```
...
for (variable in numerical_variables) {
  ...
  if (variable == "Friend.Recommendation") {
    train[categorical_variable] <- cut(
      train[, variable],
      seq(0, 100, 10)
    )
  } else {
...
roc_plot(title_pred, test$High.Income, "orange")
roc_plot(company_pred, test$High.Income, "blue", TRUE)
```
...
model <- naiveBayes(as.formula(f), train)
```
