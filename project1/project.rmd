...
# prevent using scientific notation when plotting
options(scipen = 100)
# Allow sf package to be installed successfully
options(timeout = 600)
...
Some columns just have a really long chunk of text `Job Descriptions` and `URL` in particular, which are not that useful so we can drop them entirely.

```{r}
df <- df[!names(df) %in% c("Job.Descriptions", "Url")]
```
...
Looking much cleaner. Now count the missing values in each variable.
...
```{r}
summary(cbind(
  which(is.na(df$Company.Size)),
  which(is.na(df$Company.Type)),
  which(is.na(df$Company.Revenue))
))
```
...
Now there are still 130 missing values for `Company Rating` and `Company Number of Rater`, again check whether they're from the same observations.

```{r}
any(is.na(df$Company.Rating) != is.na(df$Companny.Number.of.Rater))
```

Not a single `NA` in `Company Rating` is different from the ones in `Company Number of Rater` which means they are indeed from the same observations. This makes sense because if the number of rater doesn't exist then there shouldn't be any ratings either, so the missing values in `Company Number of Rater` can be replaced with 0 indicating no one rated and the rating is an approximation based off the mean rating.

```{r}
df$Companny.Number.of.Rater[is.na(df$Companny.Number.of.Rater)] <- 0
```
...
Since the `Company Rating` is just a number out of 5 we can replace them with the mean value, similarly for `Career Opportinities`, `Compensation and Benefits`, `Company Culture and Values`, `Company Senior Management` and `Work Life Balance`. With regards to `Company Friend Reccomendation`, it is just another rating but out of 100, we can do the same for all of them.
...
ggplot(
  ...
  annotate("text", x = 12, y = 220, label = "This is not a city") +
  annotate("segment", x = 12, y = 150, xend = 12, yend = 80, arrow = arrow(
...
Locations where there are more than 10 number of jobs offered from the highest to the lowest. The 6th location is `Australia` should be changed to `Unknown` as all the other locations are cities other than countries. The reason for this category to appear could either be there are multiple locations in Australia or not sure the exact location.
...
Let's find out who the outliers are that offer such high income.
...
print(
  head(df_salary[order(-df_salary$Estimate.Base.Salary), ], 10),
  row.names = FALSE
)
```
...
As suspected all the other criteria seem to be linearly correlated to the `Company Rating`.
